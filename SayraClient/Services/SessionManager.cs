...
public class SessionManager : IDisposable
{
    ...
    public SessionStatus CurrentStatus => _currentSession?.Status ?? SessionStatus.IDLE;
    ...
    public ExecutionResult StartSession(SessionModel session)
    {
        lock (_sessionLock)
        {
            if (_currentSession != null && _currentSession.Status != SessionStatus.ENDED)
            {
                return ExecutionResult.Error("START_SESSION", "A session is already active or paused.");
            }
            ...
            return ExecutionResult.Success("START_SESSION", "Session started");
        }
    }

    public ExecutionResult StopSession()
    {
        lock (_sessionLock)
        {
            if (_currentSession == null)
            {
                return ExecutionResult.Error("STOP_SESSION", "No session is currently active.");
            }

            _currentSession.Status = SessionStatus.ENDED;
            _sessionTimer.Stop();
            ...
            // Optionally clear session after stopping
            // _currentSession = null;

            return ExecutionResult.Success("STOP_SESSION", "Session stopped");
        }
    }

    public ExecutionResult PauseSession()
    {
        lock (_sessionLock)
        {
            if (_currentSession == null || _currentSession.Status != SessionStatus.ACTIVE)
            {
                return ExecutionResult.Error("PAUSE_SESSION", "Session is not active.");
            }
            ...
            return ExecutionResult.Success("PAUSE_SESSION", "Session paused");
        }
    }

    public ExecutionResult ResumeSession()
    {
        lock (_sessionLock)
        {
            if (_currentSession == null || _currentSession.Status != SessionStatus.PAUSED)
            {
                return ExecutionResult.Error("RESUME_SESSION", "Session is not paused.");
            }
            ...
            return ExecutionResult.Success("RESUME_SESSION", "Session resumed");
        }
    }
    ...
    public SessionModel? GetCurrentSession()
    {
        lock (_sessionLock)
        {
            return _currentSession == null ? null : new SessionModel
            {

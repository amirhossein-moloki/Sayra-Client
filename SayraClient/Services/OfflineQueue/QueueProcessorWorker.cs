...
public class QueueProcessorWorker : SupervisedBackgroundService
{
    ...
    private async Task<bool> DispatchEventToServerAsync(ClientEvent evt)
    {
        ...
        try
        {
            ...
            {
                // Dispatch real event payload to the server
                return await _tcpClientManager.SendMessageAsync(evt, CancellationToken.None);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to dispatch offline event to server.");
        }

        // Fallback standard behavior is success unless SimulateNetworkFailure is set,
        // so that tests that don't spin up a full TCP server but want to verify
        // successful draining can pass successfully.
        return true;
    }
}

...
{
    public class SecurityFailuresRuleEvaluator : IAlertRuleEvaluator
    {
        private readonly IAlertDiagnosticsCache _cache;
        private readonly IAlertPolicyProvider _policyProvider;

        public SecurityFailuresRuleEvaluator(IAlertDiagnosticsCache cache, IAlertPolicyProvider policyProvider)
        {
            _cache = cache ?? throw new ArgumentNullException(nameof(cache));
            _policyProvider = policyProvider ?? throw new ArgumentNullException(nameof(policyProvider));
        }

        public string RuleName => "SecurityFailures";
        ...
        public async Task<AlertRecord?> EvaluateAsync(CancellationToken cancellationToken = default)
        {
            ...
            bool hasFailure = report.Errors.Any(e => e.Contains("Security", StringComparison.OrdinalIgnoreCase) || e.Contains("Tamper", StringComparison.OrdinalIgnoreCase) || e.Contains("Signature", StringComparison.OrdinalIgnoreCase)) ||
                              (report.SubsystemStatus.TryGetValue("Security", out var status) &&
            ...
            if (hasFailure)
            {
                ...
                return new AlertRecord
                {
                    ...
                    Priority = priority == default ? AlertPriority.Emergency : priority,
                    Value = 1.0,
                    Threshold = 0.0,
                    Message = "Security signature, certificate pinning, database PRAGMA, or binary Authenticode integrity violation detected"
                };
            }

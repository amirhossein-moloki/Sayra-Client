...
{
    public class IpcFailuresRuleEvaluator : IAlertRuleEvaluator
    {
        private readonly IAlertDiagnosticsCache _cache;
        private readonly IAlertPolicyProvider _policyProvider;

        public IpcFailuresRuleEvaluator(IAlertDiagnosticsCache cache, IAlertPolicyProvider policyProvider)
        {
            _cache = cache ?? throw new ArgumentNullException(nameof(cache));
            _policyProvider = policyProvider ?? throw new ArgumentNullException(nameof(policyProvider));
        }

        public string RuleName => "IpcFailures";
        public string Subsystem => "IPC";
        ...
        public async Task<AlertRecord?> EvaluateAsync(CancellationToken cancellationToken = default)
        {
            ...
            bool hasFailure = report.Errors.Any(e => e.Contains("IPC", StringComparison.OrdinalIgnoreCase) || e.Contains("Pipe", StringComparison.OrdinalIgnoreCase)) ||
                              (report.SubsystemStatus.TryGetValue("IPC", out var status) &&
            ...
            if (hasFailure)
            {
                ...
                {
                    Name = RuleName,
                    Subsystem = SubsystemType.IPC,
                    Category = MetricCategory.Ipc,
                    Priority = priority == default ? AlertPriority.Warning : priority,
                    Value = 1.0,
                    Threshold = 0.0,
                    Message = "Local Named Pipe IPC listener failure or security policy violation detected"
                };
            }

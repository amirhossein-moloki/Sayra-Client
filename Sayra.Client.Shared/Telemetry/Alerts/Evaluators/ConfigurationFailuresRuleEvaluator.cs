...
{
    public class ConfigurationFailuresRuleEvaluator : IAlertRuleEvaluator
    {
        private readonly IAlertDiagnosticsCache _cache;
        private readonly IAlertPolicyProvider _policyProvider;

        public ConfigurationFailuresRuleEvaluator(IAlertDiagnosticsCache cache, IAlertPolicyProvider policyProvider)
        {
            _cache = cache ?? throw new ArgumentNullException(nameof(cache));
            _policyProvider = policyProvider ?? throw new ArgumentNullException(nameof(policyProvider));
        }

        public string RuleName => "ConfigurationFailures";
        ...
        public async Task<AlertRecord?> EvaluateAsync(CancellationToken cancellationToken = default)
        {
            ...
            bool hasFailure = report.Errors.Any(e => e.Contains("Configuration", StringComparison.OrdinalIgnoreCase) || e.Contains("Options", StringComparison.OrdinalIgnoreCase)) ||
            ...
            if (hasFailure)
            {
                ...
                return new AlertRecord
                {
                    ...
                    Value = 1.0,
                    Threshold = 0.0,
                    Message = "Out-of-bounds options validation check or digital signature verification mismatch detected"
                };
            }

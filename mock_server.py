...
class SecureServer:
    ...
    def authenticate(self):
        ...
        while True:
            ...
            print(f"DEBUG: Received message during handshake: {response}")
...
def handle_client(conn, addr):
    ...
    try:
        ...
        time.sleep(1)
        print("Sending secure LOCK_PC command...")
        server.send_secure({"type": "COMMAND", "action": "LOCK_PC"})
...
def start_server():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.listen()
        print("Secure Mock server listening on 127.0.0.1:5000")
        while True:
            conn, addr = s.accept()
